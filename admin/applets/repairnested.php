...
function checkNestedSet($table) {
    ...
    if (!cmsCore::c('db')->errno()) {
        $errors[] = (cmsCore::c('db')->num_rows($res)>0);
    } else {
        $errors[] = true;
    }
...
function repairNestedSet($table) {
    ...
    $sql = "SELECT id FROM ". $table ." WHERE NSDiffer = '". $differ ."' AND NSLevel > 0 ORDER BY NSLeft";
    $res = cmsCore::c('db')->query($sql);
...
function applet_repairnested() {
    ...
    $tables = array(
        array(
            'name'   => 'cms_category',
            'title'  => $_LANG['AD_ARTICLES_TREE'],
            'differ' => '',
            'error'  => checkNestedSet('cms_category')
        ),
        array(
            'name'   => 'cms_photo_albums',
            'title'  => $_LANG['AD_ALBUMS_TREE'],
            'differ' => '',
            'error'  => checkNestedSet('cms_photo_albums')
        ),
        array(
            'name'   => 'cms_board_cats',
            'title'  => $_LANG['AD_DESK_TREE'],
            'differ' => '',
            'error'  => checkNestedSet('cms_board_cats')
        ),
        array(
            'name'   => 'cms_uc_cats',
            'title'  => $_LANG['AD_CATALOG_TREE'],
            'differ' => '',
            'error'  => checkNestedSet('cms_uc_cats')
        ),
        array(
            'name'   => 'cms_menu',
            'title'  => $_LANG['AD_MENU_TREE'],
            'differ' => '',
            'error'  => checkNestedSet('cms_menu')
        ),
        array(
            'name'   => 'cms_forums',
            'title'  => $_LANG['AD_FORUMS_TREE'],
            'differ' => '',
    ...
    if ($inCore->isComponentInstalled('maps')) {
        $tables[] = array(
            'name'   => 'cms_map_cats',
            'title'  => $_LANG['AD_MAPS_TREE'],
            'differ' => '',
            'error'  => checkNestedSet('cms_map_cats')
        );
    }
    ...
    if ($inCore->isComponentInstalled('video')) {
        $tables[] = array(
            ...
            'title'  => $_LANG['AD_VIDEO_TREE'],
            'differ' => '',
            'error'  => checkNestedSet('cms_video_category')
        );
    }
    ...
    if ($inCore->isComponentInstalled('shop')) {
        $tables[] = array(
            'name'   => 'cms_shop_cats',
            'title'  => $_LANG['AD_SHOP_TREE'],
            'differ' => '',
            'error'  => checkNestedSet('cms_shop_cats')
        );
    }
    ...
    cmsCore::c('page')->initTemplate('applets', 'repairnested')->
        display();
