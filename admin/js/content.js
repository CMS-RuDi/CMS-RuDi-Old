function sendContentForm(opt, object_id){
    ...
    if (sel){
        if (opt == 'delete') {
            adminAlert(LANG_AD_DELETE_SELECTED_ARTICLES+' ('+sel+' '+LANG_AD_PIECES+')?', false, {ok: function(){ document.selform.action = link; document.selform.submit(); }, cancel_text: LANG_CANCEL});
        } else {
...
function deleteCat(cat_name, cat_id){
    adminAlert(
        LANG_AD_CATEGORY_DELETE+' "'+cat_name+'" '+LANG_AD_AND_SUB_CATS,
        false,
        {
            ok: function(){
                adminAlert(
                    LANG_AD_DELETE_SUB_ARTICLES,
                    false,
                    {
                        ok: function() {
                            window.location.href = '?view=cats&do=delete&id='+cat_id+'&content=1';
                        },
                        cancel: function() {
                            window.location.href = '?view=cats&do=delete&id='+cat_id;
                        },
                        cancel_text: LANG_CANCEL
                    }
                );
            },
            cancel_text: LANG_CANCEL
