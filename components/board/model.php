...
class cms_model_board {
    ...
    public function getCommentTarget($target, $target_id) {
        ...
        switch($target) {
            case 'boarditem': $item = cmsCore::c('db')->get_fields('cms_board_items', "id='". $target_id ."'", 'title, obtype');
                              if (!empty($item)) {
    ...
    public function whereCatIs($cat_id) {
        cmsCore::c('db')->where("i.category_id = '". $cat_id ."'");
        return;
    }
    ...
    public function whereCityIs($city) {
        cmsCore::c('db')->where("LOWER(i.city) = '". $city ."'");
    }

    public function whereVip($flag) {
        cmsCore::c('db')->where("i.is_vip = ". $flag);
    }

    public function whereTypeIs($type) {
        cmsCore::c('db')->where("LOWER(i.obtype) = '". $type ."'");
    }

    public function whereUserIs($user_id) {
        cmsCore::c('db')->where("i.user_id = '". $user_id ."'");
    }
    ...
    public function getCategory($category_id = 0) {
        if ($category_id == $this->root_cat['id']) {
            ...
            $category = cmsCore::c('db')->get_fields('cms_board_cats', "id = '". $category_id ."'", '*');
        }
        ...
        if (!$category['obtypes']) {
            $category['obtypes'] = cmsCore::c('db')->get_field('cms_board_cats', "NSLeft <= ". $category['NSLeft'] ." AND NSRight >= ". $category['NSRight'] ." AND obtypes <> ''", 'obtypes');
    ...
    public function getSubCats($category_id) {
        ...
        while ($cat = cmsCore::c('db')->fetch_assoc($result)) {
            if (!$cat['obtypes']) {
                $cat['obtypes'] = cmsCore::c('db')->get_field('cms_board_cats', "NSLeft <= ". $cat['NSLeft'] ." AND NSRight >= ". $cat['NSRight'] ." AND obtypes <> ''", 'obtypes');
            }
    ...
    public function getRecord($item_id) {
        ...
        $record = cmsCore::c('db')->fetch_assoc($result);
    ...
    public function deleteRecord($item_id) {
        ...
        cmsCore::c('db')->delete('cms_board_items', " id = '". $item_id ."'", 1);
    ...
    public function clearOldVips() {
        return cmsCore::c('db')->query("UPDATE cms_board_items SET is_vip=0 WHERE DATE(vipdate) <= CURRENT_DATE");
    }

    public function deleteVip($item_id) {
        return cmsCore::c('db')->query("UPDATE cms_board_items SET is_vip=0 WHERE id = '". $item_id ."'");
    }
    ...
    public function getOrder($order='', $default='') {
        ...
        } else if (cmsUser::sessionGet('ad_'. $order)) {
    ...
    public function getCatCity($cat_id = 0) {
        ...
        if (cmsCore::c('db')->num_rows($result)) {
            while ($c = cmsCore::c('db')->fetch_assoc($result)) {
                $cat_city[] = $c['city'];
            }
        }
    ...
    public function orderForm($orderby, $orderto, $category) {
        return cmsPage::initTemplate('components', 'com_board_order_form')->
    ...
    public function checkLoadedByUser24h($cat) {
        ...
        if (cmsCore::c('user')->id) {
            $where = " AND user_id = '". cmsCore::c('user')->id ."'";
        } else if (cmsCore::c('user')->ip && cmsCore::c('user')->ip != '127.0.0.1'){
            $where = " AND ip = INET_ATON('". cmsCore::c('user')->ip ."')";
        } else {
            return false;
        }

        $u_count = cmsCore::c('db')->rows_count('cms_board_items', "category_id = '". $cat['id'] ."' ". $where ." AND pubdate >= DATE_SUB(NOW(), INTERVAL 1 DAY)");
    ...
    public function checkAccess($user_id) {
        if (cmsCore::c('user')->id) {
            $access = (cmsCore::c('user')->is_admin || $this->is_moderator_by_group || $user_id == cmsCore::c('user')->id);
        } else {
            $access = false;
        }
    ...
    public function checkAdd($cat){
        ...
        if (cmsCore::c('user')->is_admin) { return true; }
    ...
    public function checkPublished($cat, $is_edit = false) {
        ...
        if(cmsCore::c('user')->is_admin || $this->is_moderator_by_group) { return 1; }
