...
class cms_model_catalog {
    ...
    public function getCommentTarget($target, $target_id) {
        ...
        switch ($target) {
            case 'catalog': $item = cmsCore::c('db')->get_fields('cms_uc_items', "id=". $target_id, 'title');
    ...
    public function deleteItem($id) {
        ...
        cmsCore::c('db')->query("DELETE FROM cms_uc_items WHERE id= '". $id ."'");
        cmsCore::c('db')->query("DELETE FROM cms_tags WHERE target='catalog' AND item_id = '". $id ."'");
        cmsCore::c('db')->query("DELETE FROM cms_uc_ratings WHERE item_id = '". $id ."'");
    ...
    public function updateItem($id, $item) {
        ...
        cmsCore::c('db')->update('cms_uc_items', $item, $id);
    ...
    public function getItemImageUrl($id) {
        $imageurl = cmsCore::c('db')->get_field('cms_uc_items', 'id='. $id, 'imageurl');
    ...
    public function addItem($item) {
        ...
        $item_id = cmsCore::c('db')->insert('cms_uc_items', $item);
    ...
    public function copyItem($id, $copies) {
        ...
        $item = cmsCore::c('db')->get_fields('cms_uc_items', 'id = '. $id, 'category_id, title, pubdate, published, imageurl, fieldsdata, is_comments, tags, rating, meta_desc, meta_keys, price, canmany, user_id, on_moderate');
        if (!$item) { return false; }
        ...
        for ($c=1; $c<=$copies; $c++) {
            ...
            cmsCore::c('db')->query('INSERT INTO cms_uc_items SET '. $set);

            $id = cmsCore::c('db')->get_last_id('cms_uc_items');
    ...
    public function updateDiscount($id, $item) {
        ...
        return cmsCore::c('db')->update('cms_uc_discount', $item, $id);
    }

    public function addDiscount($item) {
        $item = cmsCore::callEvent('ADD_CATALOG_DISCOUNT', $item);
        return cmsCore::c('db')->insert('cms_uc_discount', $item);
    }
    ...
    public function deleteCategory($id) {
        ...
        cmsCore::c('db')->deleteNS('cms_uc_cats', $id);
        return true;
    }

    public function updateCategory($id, $cat) {
        $old = cmsCore::c('db')->get_fields('cms_uc_cats', "id='". $id ."'", 'parent_id, NSLevel');
        if (!$old) { return false; }
        ...
        return cmsCore::c('db')->update('cms_uc_cats', $cat, $id);
    }
    ...
    public function copyCategory($id, $copies) {
        ...
        $item = cmsCore::c('db')->get_fields('cms_uc_cats', "id = '". $id ."'", 'parent_id, title, description, published, fieldsstruct, view_type, fields_show, showmore, perpage, showtags, showsort,	is_ratings,	orderby, orderto, showabc, shownew, newint,	filters, is_shop, is_public, can_edit, cost');
        if (!$item) { return false; }

        // Получаем родительскую категорию
        $rootid = cmsCore::c('db')->get_field('cms_uc_cats', "id = '". $item['parent_id'] ."'", 'id');
        ...
        for ($c=1; $c<=$copies; $c++) {
            ...
            $set = rtrim($set, ',');
            cmsCore::c('db')->query("UPDATE cms_uc_cats SET ". $set ." WHERE id = '". $cat_id ."' LIMIT 1");
        }
    }
    ...
    public function getSubCats($parent_id, $left_key, $right_key) {
        ...
        while ($subcat = cmsCore::c('db')->fetch_assoc($result)) {
            ...
            $count_result = cmsCore::c('db')->query($count_sql);

            $subcat['content_count'] = cmsCore::c('db')->num_rows($count_result);
    ...
    public function checkCategoryAccess($cat_id, $cat_public, $group_id) {
        return ($cat_public && cmsCore::c('db')->rows_count('cms_uc_cats_access', "cat_id=". $cat_id ." AND group_id=". $group_id, 1));
    }
