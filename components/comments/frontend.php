...
function comments($target='', $target_id=0, $labels=array(), $can_delete=null) {
	...
	if ($need_captcha && !cmsCore::checkCaptchaCode()) {
	...
	if ($comment['published']) {
            ...
            cmsUser::sendUpdateNotify($comment['target'], $comment['target_id'], array(
                'link'        => $comment['target_link'] .'#c'. $comment_id,
	...
	if ($model->config['email']) {
            $mailmsg = str_replace(
                array('{sitename}', '{date}', '{from}', '{subjtitle}', '{targetlink}', '{content}'),
                array($inConf->sitename, date('d/m/Y (H:i)'), $from_nick, stripslashes($comment['target_title']), $targetlink, strip_tags($comment['content'])),
	...
	if (!$model->is_can_moderate && !$inUser->is_admin && !(($inUser->id == $comment['user_id']) && $comment['is_editable'])){ cmsCore::error404(); }
	...
	if (isset($_SESSION['comments'][$comment['target']][$comment['target_id']])) {
        ...
        if (!($model->is_can_delete && $inUser->id == $comment['user_id']) && !$model->is_can_moderate && !$inUser->is_admin && !$can_delete) {
            cmsCore::error404();
        }
