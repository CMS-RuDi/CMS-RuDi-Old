...
class cms_model_photos {
    ...
    public function getCommentTarget($target, $target_id) {
        ...
        switch($target){
            case 'palbum': $album = cmsCore::c('db')->get_fields('cms_photo_albums', "id='". $target_id ."'", 'title');
                           if (!$album) { return false; }
                           $result['link']  = '/photos/'. $target_id;
                           $result['title'] = $album['title'];
                           break;
            case 'photo':  $photo = cmsCore::c('db')->get_fields('cms_photo_files', "id='". $target_id ."'", 'title');
    ...
    public function updateRatingHook($target, $item_id, $points) {
        ...
        switch($target){
            case 'photo': $sql = "UPDATE cms_photo_files SET rating = rating + (". $points .") WHERE id = '". $item_id ."'";
                break;
        }

        if (!empty($sql)) { cmsCore::c('db')->query($sql); }
    ...
    public function loadedByUser24h($user_id, $album_id) {
        return cmsCore::c('db')->rows_count('cms_photo_files', "user_id = '". $user_id ."' AND album_id = '". $album_id ."' AND pubdate >= DATE_SUB(NOW(), INTERVAL 1 DAY)");
    }
}