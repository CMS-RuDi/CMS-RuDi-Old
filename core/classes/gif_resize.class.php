...
class gifresizer {
    ...
    */ 
    public function resize($orgfile,$newfile,$width,$height){
        $this->decode($orgfile);
    ...
    */
    private function writeframes($prepend){
        for($i=0; $i<sizeof($this->imagedata); $i++){
            file_put_contents(PATH . $this->temp_dir ."/frame_". $prepend ."_". str_pad($i, 2, "0", STR_PAD_LEFT) .".gif", $this->imageinfo["gifheader"] . $this->imagedata[$i]["graphicsextension"] . $this->imagedata[$i]["imagedata"] .chr(0x3b));
            $this->parsedfiles[] = PATH . $this->temp_dir ."/frame_". $prepend ."_". str_pad($i,2,"0",STR_PAD_LEFT) .".gif";
        }
    }
