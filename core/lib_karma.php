...
function cmsKarma($target, $item_id){ //returns array with total votes and total points of karma
    ...
    $item = cmsCore::c('db')->get_fields('cms_ratings_total', "item_id = '$item_id' AND target='$target'", 'total_rating, total_votes');
...
function cmsAlreadyKarmed($target, $item_id, $user_id){
    return cmsCore::c('db')->rows_count('cms_ratings', "target='$target' AND item_id = $item_id AND user_id = '$user_id'");
}

function cmsAlreadyKarmedIP($target, $item_id, $ip){
    return cmsCore::c('db')->rows_count('cms_ratings', "target='$target' AND item_id = $item_id AND ip = '$ip'");
}
...
function cmsSubmitKarma($target, $item_id, $points){
    ...
    if(cmsAlreadyKarmed($target, $item_id, cmsCore::c('user')->id)){ return false; }
    ...
    $is_agr = cmsCore::c('db')->rows_count('cms_ratings_total', "target='$target' AND item_id = '$item_id'", 1);
    ...
    cmsCore::c('db')->query($agr_sql);

    //получаем информацию о цели
    $info = cmsCore::c('db')->get_fields('cms_rating_targets', "target='{$target}'", '*');
    ...
    if ($info['component']){
        ...
        if (class_exists('cms_model_'.$info['component'])){
            ...
            if (method_exists($model, 'updateRatingHook')){
                ...
                if (cmsCore::c('db')->isFieldExists($info['target_table'], 'rating')){
