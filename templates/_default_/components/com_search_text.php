...
<form id="sform"action="/search" method="GET" enctype="multipart/form-data" style="clear:both">
    ...
    <input type="text" name="query" id="query" size="40" value="<?php echo $query; ?>" class="text-input" />
    <select name="look" style="width:100px" onchange="$('form#sform').submit();	">
        ...
        <option value="anyword" <?php if ($look == 'anyword' || $look=='') { ?> selected="selected" <?php } ?>><?php echo $_LANG['ANY_WORD']; ?></option>
        <option value="phrase" <?php if ($look == 'phrase' || $look=='') { ?> selected="selected" <?php } ?>><?php echo $_LANG['PHRASE']; ?></option>
    </select>
	...
	<table width="" border="0" cellspacing="0" cellpadding="3">
        ...
        <label id="order_by_date" <?php if ($order_by_date) { ?>class="selected"<?php } ?>>
            <input name="order_by_date" onclick="toggleInput('order_by_date')" type="checkbox" value="1" <?php if ($order_by_date) { ?>checked="checked"<?php } ?> />
            <?php echo $_LANG['SORT_BY_PUBDATE']; ?></label>
...
<?php if ($results) { ?>
	...
	<div class="search_block">
            ...
            <div class="search_result_desc">
                ...
                <?php } ?>
            <a href="<?php echo $item['placelink']; ?>"><?php echo $item['place']; ?></a>
                &mdash; <span style="color:green"><?php echo $host; ?><?php echo $item['link']; ?></span>
            </div>
     </div>
    <?php $num++; ?>
    <?php } ?>
