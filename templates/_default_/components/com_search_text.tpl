<div class="photo_details">
    <form id="sform"action="/search" method="GET" enctype="multipart/form-data" style="clear:both">
        ...
        <select name="look" style="width:100px" onchange="$('form#sform').submit();	">
                ...
                <option value="anyword" {if $look == 'anyword'} selected="selected" {/if}>{$LANG.ANY_WORD}</option>
                <option value="phrase" {if $look == 'phrase'} selected="selected" {/if}>{$LANG.PHRASE}</option>
        </select>
        ...
        <div id="from_search">
            ...
            <label id="order_by_date" {if $order_by_date}class="selected"{/if}>
                <input name="order_by_date" onclick="toggleInput('order_by_date')" type="checkbox" value="1" {if $order_by_date}checked="checked"{/if} /> {$LANG.SORT_BY_PUBDATE}
            </label>
